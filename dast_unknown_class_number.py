...
def get_target_label(data):
    with torch.no_grad(): # With no gradient calculation:
        # Pass the target model the data and get the predicted labels
        if opt.dataset == 'azure':
...
for epoch in range(opt.niter): # For each epoch:
    ...
    for ii in range(batch_num): # For each batch:
        ...
        for i in range(len(noise_chunk)):
            ...
            target_labels, target_outputs = get_target_label(gene_data)
            ...
            # Concatenate the generated training data and their intended labels &
            # concatenate the target model's predicted labels and probabilities for the generated training data
            if i == 0:
                data = gene_data # Initialize the generated training data with the first batch of generated data
                set_label = intended_label # Assign the intended label for the generated training data
                label = target_labels # Get the target model's predicted labels for the generated training data
                outputs = target_outputs # Get the target model's predicted probabilities for the generated training data
            else:
                data = torch.cat((data, gene_data), 0) # Concatenate each batch of generated training data together
                set_label = torch.cat((set_label, intended_label), 0) # Concatenate the intended labels for the generated training data together
                label = torch.cat((label, target_labels), 0) # Concatenate the target model's predicted labels together
                outputs = torch.cat((outputs, target_outputs), 0) # Concatenate the target model's predicted probabilities together
